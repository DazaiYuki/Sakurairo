...
1. ⚠️ **恢复区块样式按需加载（T2）—— 已实测，须你决策**
   体积确实省 **137.5 KB / −38%**（首页样式表 4 个 → 3 个），
   但视觉回归显示**文章页高度变化 2~24px**，是真实的视觉变更。
   二选一：接受外观变化（另开"允许视觉变更"阶段）／恢复按需加载同时把主题正文字式
   补全到不依赖区块库（工作量大易漏）。详见文档 02 的 §4.2
2. `$wpdb` 33 处查询复核（安全，只读）
3. 内联 `<script>` / `<style>` 外链化
4. 评估移除 `polyfill.js`
5. ~~主题 CSS 静态化~~ —— ⚠️ **收益已更正**：本地实测 PHP 拼接+压缩仅 5~20ms，
   不是 TTFB 问题。线上那 2.30s 更可能是主机侧对带查询串 URL 的缓存策略所致，
   需先在主机上确认成因，详见文档 02 的 §4.1
